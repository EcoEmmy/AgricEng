...
if not db_url:
    app.logger.warning("DATABASE_URL not set. Please provide a valid PostgreSQL connection string.")
    app.logger.warning("Format: postgresql://username:password@host:port/database_name")
...
# =========================================================
USE_SQLITE_FALLBACK = os.environ.get('USE_SQLITE_DEV', 'false').lower() == 'true'
...
def seed_database():
    ...
    try:
        with conn.cursor() as cur:
            ...
            if admin_count == 0:
                # Use environment variable for initial admin password or generate random one
                initial_password = os.environ.get('INITIAL_ADMIN_PASSWORD', 'ChangeMeOnFirstLogin2024!')
                ...
                app.logger.warning("⚠️  Default admin created. Username: admin")
                app.logger.warning("⚠️  CHANGE THE PASSWORD IMMEDIATELY after first login!")
                app.logger.warning("⚠️  Set INITIAL_ADMIN_PASSWORD env var for custom initial password")
...
def student_dashboard():
    """Student dashboard showing their results and payment status."""
    student = get_current_student()
    if not student:
        return redirect(url_for('student_login'))
    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            has_payment = payment and payment['status'] == 'approved'
            ...
            # Get results only if payment is approved
            results = []
            if has_payment:
                cur.execute("""
        ...
        for result in results:
            total_grade_points += float(result['grade_point']) * result['course_unit']
            total_units += result['course_unit']
        
        cgpa = round(total_grade_points / total_units, 2) if total_units > 0 else 0.0
